...
function FieldGLMap({ className }: MapType) {
    ...
    if (field.isLoading || field.data.message === undefined || gates.isLoading || gates.data.message === undefined) {
        return <ClipLoader />;
    }
    ...
    if (field.data.status === "200") {
        ...
        return (
            <div className={"relative border-4 border-solid rounded-3xl border-Corp3 " + className}>
                <Map
                    onClick={(e) => {
                        addGate
