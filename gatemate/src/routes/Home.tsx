...
function Header() {
    ...
    if (fayettevilleWeather.isLoading || fayettevilleWeather === undefined || simsboroWeather.isLoading || simsboroWeather === undefined || magnoliaWeather.isLoading || magnoliaWeather === undefined) {
        return <ClipLoader/>
    }  
        ...
        <div className={'flex flex-row gap-2 font-Arvo font-bold'}>
                ...
                <WeatherBanner className={'basis-3/4 flex flex-row gap-8 '}>
                    ...
                    <div className={'flex flex-col items-center gap-1 text-xs'}>
                        <button className={`border border-solid border-gray-500 rounded-xl p-2 hover:bg-blue-500 ${weatherArea === "Fayetteville" ? 'bg-blue-500': ''}`}onClick={() => setWeatherArea("Fayetteville")}>Fayetteville, AR</button>
                        <button className={`border border-solid border-gray-500 rounded-xl p-2 hover:bg-blue-500 ${weatherArea === "Simsboro" ? 'bg-blue-500': ''}`} onClick={() => setWeatherArea("Simsboro")}>Simsboro, AR    </button>
                        <button className={`border border-solid border-gray-500 rounded-xl p-2 hover:bg-blue-500 ${weatherArea === "Magnolia" ? 'bg-blue-500': ''}`} onClick={() => setWeatherArea("Magnolia")}>Magnolia, AR    </button>
                    </div>
                    <FontAwesomeIcon icon={faGripLinesVertical} size={"2xl"}/>
