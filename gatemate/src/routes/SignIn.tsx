        ...
        return (
            ...
            <div className={'flex flex-col gap-10 p-2'}>
                ...
                {shown ? <p className='text-red-500'>Invalid credintials</p> : null}
                    <button onClick={() => {checkAuth(username,password).then((data) => {
                        if (data.message === "Login successful") {
                            console.log(data.message)
                            window.location.href = `/home?user=${username}`
                        }
                        if (data.message === "Login failed" || data === "User not found" || data === "Internal server error") {
                            console.log(data.message)
                            setShown(true);
                        }
                    })}} className='border border-solid rounded-xl p-1 max-w-[10rem] mx-auto hover:bg-blue-500 hover:border-none'>Sign In</button>
                </div>
